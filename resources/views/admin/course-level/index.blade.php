...
@section('description', 'Course Level')
@section('breadcrumbs')
	@php echo \Breadcrumbs::render(['Course Level']) @endphp
@endsection
@section('button')
	<a href="{{ url('/admin/course-level/create') }}" class="btn btn-primary btn-xs no-border">Add Data</a>
@endsection
...
@section('content')
    ...
    <table class="table table-hover" id="course-level-table">
        <thead>
            <tr>
...
var oTable;
oTable = $('#course-level-table').DataTable({
    processing: true,
    serverSide: true,
    dom: 'lBfrtip',
    order:  [[ 0, "asc" ]],
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("admin/course-level")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
