...
@section('title', 'Student Managements')
@section('description', 'Student Managements')
@section('breadcrumbs')
	@php echo \Breadcrumbs::render(['Student Managements']) @endphp
@endsection
@section('button')
	<a href="{{ url('/admin/student/create') }}" class="btn btn-primary btn-xs no-border">Add Data</a>
@endsection
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("admin/student")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
