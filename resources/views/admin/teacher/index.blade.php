...
@section('title', 'Teacher Managements')
@section('description', 'Teacher Managements')
@section('breadcrumbs')
	@php echo \Breadcrumbs::render(['Teacher Managements']) @endphp
@endsection
@section('button')
	<a href="{{ url('/admin/teacher/create') }}" class="btn btn-primary btn-xs no-border">Add Data</a>
@endsection
...
@section('content')
    ...
    <table class="table table-hover" id="teacher-table">
        <thead>
            <tr>
                <th>Unique Number</th><th>Name</th><th>Status</th><th>Last Login At</th>
...
var oTable;
oTable = $('#teacher-table').DataTable({
    processing: true,
    serverSide: true,
    dom: 'lBfrtip',
    order:  [[ 0, "asc" ]],
...
$("#teacher-table_wrapper > .dt-buttons").appendTo("div.export-options-container");
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("admin/teacher")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
